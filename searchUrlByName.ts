...
fs.readFile('.\\json-products\\diagnosis-list-updated2.json', 'utf-8', (err, data) => {
    ...
    //let list = [];
/*
Problem: If I tried to loop over the whole json file (1375 elements), it would crash. I tried other to scrape the links by myself
...
I used if condition to not search again for the once that have already links saved. Through repeating this cca 5 time for each 200
I got to find the links where it was possible. There are still some without any links in urlByName, but there are no search results
on duckduckgo. 
I think this can work without the manual part but there has to be some time delay implemented but I didn't know how.
*/
